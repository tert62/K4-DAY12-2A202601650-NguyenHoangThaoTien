...
class TokenBucket:
    ...
    def available(self, client_id: str, now: float | None = None) -> float:
        ...
        Chặn trần ``min(capacity, ...)`` là phần quan trọng nhất — thiếu nó thì
        client im lặng một ngày sẽ tích được 14.400 token và bắn hết trong một
        giây, tức là không còn giới hạn gì nữa.
        """
        ...
        last = float(state["ts"])
        # Token nhỏ thêm kể từ lần cập nhật cuối
    ...
    def consume(self, client_id: str, now: float | None = None) -> None:
        ...
        Ghi lại **cả** ``ts``. Quên cập nhật ``ts`` thì lần sau ta tính phần
        nạp thêm từ một mốc thời gian đã cũ, và xô tự đầy lại vô tội vạ.
        """
        now = now if now is not None else time.time()
