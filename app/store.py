...
class ChatStore:
    ...
    def ping(self) -> bool:
        ...
        Bắt mọi Exception (mất mạng, sai mật khẩu, Redis chưa khởi động) và
        biến thành False: probe phải trả lời được, không được nổ thành 500.
        """
        try:
            self.client.ping()
            return True
    ...
    def add_turn(self, client_id: str, role: str, content: str) -> None:
        ...
        Chỉ giữ ``HISTORY_MAX_MESSAGES`` message gần nhất — không cắt thì prompt
        phình vô hạn và tiền token cũng vậy. Hội thoại cũ tự hết hạn theo TTL.
        """
    ...
    def history(self, client_id: str) -> list[dict]:
        ...
        Chưa có gì → ``lrange`` trả list rỗng, không cần xử lý riêng.
        """
